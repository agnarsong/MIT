...
export declare namespace BVM_EigenDataLayrChain {
  ...
  export type BatchRollupBlockStructOutput = [BigNumber, BigNumber] & {
  ...
  export type RollupStoreStructOutput = [number, number, number] & {
    dataStoreId: number;
    confirmAt: number;
    status: number;
  };
...
export interface BVM_EigenDataLayrChainInterface extends utils.Interface {
  functions: {
    ...
    "confirmData(bytes,((bytes32,uint32,uint32,uint32,uint96,address,bytes32),uint8,uint256,uint32),uint256,uint256)": FunctionFragment;
    ...
    "storeData(bytes,uint8,uint32,uint256,uint256,uint32)": FunctionFragment;
